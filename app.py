...
SUP_FILE_PATH = os.path.join(BASE, "_sup.json")
BADGES_FILE_PATH = os.path.join(BASE, "_badges.json")

def read_json_from_file(filepath, default=None):
    """Generic: read JSON from file"""
    try:
...
def init_db():
    ...
    if cur.fetchone()[0] == 0:
        defaults = {
            ...
            "friday": ["17:00","17:15","17:30","17:45","18:00","18:15","18:30","18:45","19:00","19:15","19:30"]
        }
    ...
    # Restore sup-list from file if available
    file_sup = read_json_from_file(SUP_FILE_PATH)
    if file_sup is not None:
        con.execute("INSERT OR REPLACE INTO store(key,val) VALUES(?,?)", ("sup_list", json.dumps(file_sup)))
    # Restore badges from file if available  
    file_badges = read_json_from_file(BADGES_FILE_PATH)
    if file_badges is not None:
        for name, code in file_badges.items():
...
def api_data():
    ...
    if request.method == "GET":
        ...
        cdur_basic_row = con.execute("SELECT val FROM store WHERE key='cdur_basic'").fetchone()
        cdur_basic = json.loads(cdur_basic_row["val"]) if cdur_basic_row else {}
        cdur_adv_row = con.execute("SELECT val FROM store WHERE key='cdur_adv'").fetchone()
        cdur_adv = json.loads(cdur_adv_row["val"]) if cdur_adv_row else {}

        con.close()
        return jsonify({"employees":emps,"assignments":ass,"shifts":sh,"view_only":vo,"custom_durations":cdur,"cdur_basic":cdur_basic,"cdur_adv":cdur_adv,"recipients":rs,"durations":dur,"extra_breaks":xb,"extra_breaks_req":xb_req,"mode":mode,"advanced_slots":ADV_SLOTS})
        ...
        if old_ass and not new_ass:
            ...
            if not old_row:
                ...
                dp = con.execute("SELECT * FROM duration_pattern WHERE id=1").fetchone()
                dur = [dp["d0"],dp["d1"],dp["d2"],dp["d3"]]
                sh = {"morning":{"lb":"🌅 Morning (8-5)"},"afternoon":{"lb":"☀️ Afternoon (11-8)"},"night":{"lb":"🌙 Night (1-10)"},"friday":{"lb":"🎯 الجمعة (4-7:30)"}}
                for r in con.execute("SELECT * FROM shifts ORDER BY shift, idx"):
        ...
        if d.get("cdur_basic") is not None:
            con.execute("INSERT OR REPLACE INTO store(key,val) VALUES(?,?)", ("cdur_basic", json.dumps(d["cdur_basic"])))
        if d.get("cdur_adv") is not None:
            con.execute("INSERT OR REPLACE INTO store(key,val) VALUES(?,?)", ("cdur_adv", json.dumps(d["cdur_adv"])))
...
def api_badges():
    """Get or set badge codes with file persistence (survives Render restarts)"""
    if request.method == "GET":
        file_data = read_json_from_file(BADGES_FILE_PATH)
        ...
        d = request.json
        if not d: return jsonify({"error":"no data"}), 400
        if "name" in d and "code" in d:
            # Single badge action
            con = get_db()
            con.execute("UPDATE employees SET code=? WHERE name=?", (str(d["code"]), d["name"]))
            con.commit()
            con.close()
            # Save to file
            badges = read_json_from_file(BADGES_FILE_PATH) or {}
            badges[d["name"]] = str(d["code"])
            write_json_to_file(BADGES_FILE_PATH, badges)
            return jsonify({"ok":True, "message":"✅ تم حفظ البادج"})
        # Bulk update
        con = get_db()
        ...
        con.commit()
        con.close()
        write_json_to_file(BADGES_FILE_PATH, d)
...
def api_sup_list():
    """Get/set supervisor list with file persistence (survives Render restarts)"""
    if request.method == "GET":
        # Try file first, fallback to DB
        file_data = read_json_from_file(SUP_FILE_PATH)
        if file_data is not None:
            return jsonify(file_data)
        con = get_db()
        ...
        if isinstance(d, list):
            ...
            con.commit()
            con.close()
            write_json_to_file(SUP_FILE_PATH, d)
